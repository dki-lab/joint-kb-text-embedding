...
if backend.lower() == 'mxnet':
    ...
    from train_pytorch import test, test_mp, test_demo, test_mp_demo
...
class ArgParser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('--sampler-type', type=str, choices=['head', 'tail', 'both'], default='both')
...
def main():
    ...
    eval_dataset = EvalDatasetNew(dataset, args)
    ...
    if args.num_proc > 1:
        ...
        procs = []
        for i in range(args.num_proc):
            proc = mp.Process(target=test_mp_demo, args=(args,
                                                    model,
        ...
        if args.sampler_type == 'both':
            test_demo(args, model, [test_sampler_head, test_sampler_tail], id2entity_map, id2relation_map, wiki_name)
        elif args.sampler_type == 'head':
            test_demo(args, model, [test_sampler_head], id2entity_map, id2relation_map, wiki_name)
        elif args.sampler_type == 'tail':
            test_demo(args, model, [test_sampler_tail], id2entity_map, id2relation_map, wiki_name)
