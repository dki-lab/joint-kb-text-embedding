...
class KGDataset:
    ...
    * test stores the triples in the test set.
    ...
    '''
    def __init__(self, entity_path, relation_path, common_entity_nodes, train_path, 
        ...
        print('n_entities = {}'.format(self.n_entities))
        # self.n_entities = self.ent_thresh # correction for extra entities in entity2id map
        print('n_entities = {}'.format(self.n_entities))

        self.relation2id, self.n_relations = self.read_relation(relation_path)
        self.common_entity_nodes = common_entity_nodes
        # print(type(self.common_entity_nodes))
        # print('len(common_entity_nodes) = {}'.format(len(self.common_entity_nodes)))
        self.common_entity_node_ids = list(map(lambda x:self.entity2id[x], self.common_entity_nodes))

        self.train, self.train_name_graph = self.read_triple(train_path, "train", skip_first_line, format)
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0,1,2]):
        ...
        with open(path) as f:
            ...
            for line in f:
                ...
                # if h in self.common_entity_nodes:
                #     heads.append(self.entity2id[h+'-wiki'])
                #     rels.append(self.relation2id[r])
                #     tails.append(self.entity2id[t])
                # if t in self.common_entity_nodes:
                #     heads.append(self.entity2id[h])
                #     rels.append(self.relation2id[r])
                #     tails.append(self.entity2id[t+'-wiki'])
                # if h in self.common_entity_nodes and t in self.common_entity_nodes:
                #     heads.append(self.entity2id[h+'-wiki'])
                #     rels.append(self.relation2id[r])
                #     tails.append(self.entity2id[t+'-wiki'])
        ...
        if mode == "train":
            print('len(common heads) = {}'.format(len(set(heads) & set(self.common_entity_node_ids))))
            print('len(common tails) = {}'.format(len(set(tails) & set(self.common_entity_node_ids))))

            self.common_entity_node_ids = set(self.common_entity_node_ids)

            print(len(heads_new))
            common_head_ids = [i for i,h in enumerate(heads) if h in self.common_entity_node_ids]
            for triple_id in common_head_ids:
                heads_new.append(self.entity2id[self.id2entity_map[heads[triple_id]]+'-wiki'])
                rels_new.append(rels[triple_id])
                tails_new.append(tails[triple_id])

            print(len(heads_new))

            common_tail_ids = [i for i,t in enumerate(tails) if t in self.common_entity_node_ids]
            for triple_id in common_tail_ids:
                heads_new.append(heads[triple_id])
                rels_new.append(rels[triple_id])
                tails_new.append(self.entity2id[self.id2entity_map[tails[triple_id]]+'-wiki'])

            print(len(heads_new))
            common_head_tail_ids = [i for i, (h,t) in enumerate(zip(heads, tails)) if h in self.common_entity_node_ids and t in self.common_entity_node_ids]
            for triple_id in common_head_tail_ids:
                heads_new.append(self.entity2id[self.id2entity_map[heads[triple_id]]+'-wiki'])
                rels_new.append(rels[triple_id])
                tails_new.append(self.entity2id[self.id2entity_map[tails[triple_id]]+'-wiki'])
            print(len(heads_new))
        ...
        heads_new = np.array(heads_new, dtype=np.int64)
        tails_new = np.array(tails_new, dtype=np.int64)
        rels_new = np.array(rels_new, dtype=np.int64)

        print('Finished. Read {} {} triples.'.format(len(heads), mode))
        if mode == "train":
            return (heads, rels, tails), (heads_new, rels_new, tails_new)
        else:
...
class KGDatasetUDDRaw(KGDataset):
    ...
    * test stores the triples in the test set. In format [src_name, rel_name, dst_name]
    ...
    The triples are stored as 'head_nid\trelation_id\ttail_nid'.
    '''
    def __init__(self, path, name, files, format, dictionary, wiki_link_dict):
        self.name = name
        self.dictionary = dictionary
        self.wiki_link_dict = wiki_link_dict
    ...
    def load_entity_relation(self, path, files, format):
        ...
        self.id2entity_map = {value:key for key,value in entity_map.items()}

        # print(entity_map)
        common_node_ids = list(map(lambda x:x[0], filter(lambda x:x[1]!=-1, map(lambda x:(x[0], self.dictionary.get_entity_index(x[1])), filter(lambda x: x[1], map(lambda x:(x[0], self.wiki_link_dict.get(x[1], None)), list(self.id2entity_map.items())))))))
        self.common_entity_nodes = list(map(lambda x:self.id2entity_map[x], common_node_ids))
        print('len(common_entity_nodes) = {}'.format(len(self.common_entity_nodes)))

        self.ent_thresh = len(entity_map)
        print('ent thresh = {}'.format(self.ent_thresh))

        for ent in self.common_entity_nodes:
            new_ent = ent + '-wiki'
            new_ent_id = len(entity_map)
            entity_map[new_ent] = new_ent_id
            self.id2entity_map[new_ent_id] = new_ent
...
def get_dataset(data_path, data_name, format_str, dictionary, wiki_link_dict, files=None):
    if format_str == 'built_in':
        ...
        format = format_str[8:]
        dataset = KGDatasetUDDRaw(data_path, data_name, files, format, dictionary, wiki_link_dict)
