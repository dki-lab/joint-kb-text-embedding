...
from dataloader import EvalDataset, TrainDataset, TrainDatasetNameGraph, NewBidirectionalOneShotIterator
...
def main():
    ...
    train_data_name_graph = TrainDatasetNameGraph(dataset, args, ranks=args.num_proc)

    print('no. of nodes (name graph) = {}'.format(train_data_name_graph.g.number_of_nodes()))
    print('no. of edges (name graph) = {}'.format(train_data_name_graph.g.number_of_edges()))

    if args.num_proc > 1:
        train_samplers_name_graph = []
        for i in range(args.num_proc):
            # print('rank = {}'.format(i))
            train_sampler_head_name_graph = train_data_name_graph.create_sampler(args.batch_size,
                                                           args.neg_sample_size,
                                                           args.neg_sample_size,
                                                           mode='head',
                                                           num_workers=args.num_proc,
                                                           ...
                                                           exclude_positive=False,
                                                           rank=i)
            train_sampler_tail_name_graph = train_data_name_graph.create_sampler(args.batch_size,
                                                           args.neg_sample_size,
                                                           args.neg_sample_size,
                                                           mode='tail',
                                                           num_workers=args.num_proc,
                                                           ...
                                                           exclude_positive=False,
                                                           rank=i)
            train_samplers_name_graph.append(NewBidirectionalOneShotIterator(train_sampler_head_name_graph, train_sampler_tail_name_graph,
                                                                  args.neg_sample_size, args.neg_sample_size,
                                                                  True, dataset.n_entities))
        ...
        train_sampler_tail_name_graph = train_data_name_graph.create_sampler(args.batch_size,
                                                       ...
                                                       shuffle=True,
                                                       exclude_positive=False)
        train_sampler_name_graph = NewBidirectionalOneShotIterator(train_sampler_head_name_graph, train_sampler_tail_name_graph,
                                                        args.neg_sample_size, args.neg_sample_size,
                                                        True, dataset.n_entities)
    ...
    print('wikipedia word vocab size = {}'.format(dictionary.word_size))
    print('wikipedia entity vocab size = {}'.format(dictionary.entity_size))
    print('wikidata entity vocab size = {}'.format(dataset.n_entities))
    ...
    max_step_per_epoch_name_graph = int(train_data_name_graph.g.number_of_edges()/ (args.batch_size * args.num_proc))+1
    vars(args)['max_step_per_epoch_name_graph'] = max_step_per_epoch_name_graph
    print('max_step_per_epoch_name_graph = {}'.format(max_step_per_epoch_name_graph))
    ...
    # TODO: convert the common node ids to corresponding wiki versions
    common_node_ids_wiki_version = [dataset.entity2id[dataset.id2entity_map[x] + '-wiki'] for x in common_node_ids]
    common_node_ids_wiki_version_tensor = torch.tensor(common_node_ids_wiki_version)

    # print('common_node_ids_wiki_version_tensor = {}'.format(common_node_ids_wiki_version_tensor))

    common_entity_sg_idx = torch.tensor([x[1] for x in common_node_ids_dict_idx])

    # print([dictionary.get_entity_by_index(x).title for x in common_entity_sg_idx])
    # print([dataset.id2entity_map[x] for x in common_node_ids_wiki_version])
    ...
    for iter_id in range(args.start_epoch, args.n_iters):
        ...
        if args.num_proc > 1:
            ...
            for i in range(args.num_proc):
                ...
                proc = ctx.Process(target=train_mp_ke, args=(args, iter_id, False, args.reg_loss_start_epoch, args.use_input_embedding, model, sg_model, log_queue, sg_reg_optimizer_list[i], proj_layer_optimizer_list[i], id2entity_map, wiki_link_dict, train_samplers[i], valid_sampler, i, rel_parts, cross_rels, barrier))
                procs.append(proc)
                proc.start()
          ...
          train_ke(args, iter_id, False, args.reg_loss_start_epoch, args.use_input_embedding, model, sg_model, log_queue, sg_reg_optimizer_list[0], proj_layer_optimizer_list[0], id2entity_map, wiki_link_dict, train_sampler, valid_samplers, 0, rel_parts, cross_rels)
        ...
        if args.num_proc > 1:
            ...
            for i in range(args.num_proc):
                valid_sampler = None
                # proc = mp.Process(target=train_process, args=(p_id, dump_db, dictionary, tokenizer, word_list, word_count_actual, freq, args, model))
                proc = ctx.Process(target=train_mp_ke, args=(args, iter_id, True, args.reg_loss_start_epoch, args.use_input_embedding, model, sg_model, log_queue, sg_reg_optimizer_list[i], proj_layer_optimizer_list[i], id2entity_map, wiki_link_dict, train_samplers_name_graph[i], valid_sampler, i, rel_parts, cross_rels, barrier))
                procs.append(proc)
                proc.start()
                print('TransE proc {} started'.format(i))
            for i,proc in enumerate(procs):
          ...
          valid_samplers = None
          train_ke(args, iter_id, True, args.reg_loss_start_epoch, args.use_input_embedding, model, sg_model, log_queue, sg_reg_optimizer_list[0], proj_layer_optimizer_list[0], id2entity_map, wiki_link_dict, train_sampler_name_graph, valid_samplers, 0, rel_parts, cross_rels)
        ...
        sg_model.emb0_lookup.weight.data[common_entity_sg_idx] = model.entity_emb.emb[common_node_ids_wiki_version_tensor]
        ...
        model.entity_emb.emb[common_node_ids_wiki_version_tensor] = sg_model.emb0_lookup.weight.data[common_entity_sg_idx]
