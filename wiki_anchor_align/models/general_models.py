...
class KEModel(object):
    ...
    def __init__(self, args, model_name, n_entities, n_relations, hidden_dim, gamma,
        ...
        self.model_name = model_name
        self.head_neg_score = self.score_func.create_neg(True) # replaced by the method below
        self.tail_neg_score = self.score_func.create_neg(False) # replaced by the method below
        self.head_neg_prepare = self.score_func.create_neg_prepare(True) # replaced by the method below
        self.tail_neg_prepare = self.score_func.create_neg_prepare(False) # replaced by the method below
        
        self.proj_layer = nn.Linear(self.entity_dim, self.entity_dim)
        self.reset_parameters()
    '''
    def head_neg_score(self, heads, relations, tails, num_chunks, chunk_size, neg_sample_size):
        hidden_dim = heads.shape[1]
        heads = heads.reshape(num_chunks, neg_sample_size, hidden_dim)
        tails = tails - relations
        tails = tails.reshape(num_chunks, chunk_size, hidden_dim)
        if self.score_func.dist_ord == 1:
            return self.score_func.gamma - batched_l1_dist(tails, heads)
        else:
            return self.score_func.gamma - batched_l2_dist(tails, heads)

    def tail_neg_score(self, heads, relations, tails, num_chunks, chunk_size, neg_sample_size):
        hidden_dim = heads.shape[1]
        heads = heads + relations
        heads = heads.reshape(num_chunks, chunk_size, hidden_dim)
        tails = tails.reshape(num_chunks, neg_sample_size, hidden_dim)
        if self.score_func.dist_ord == 1:
            return self.score_func.gamma - batched_l1_dist(heads, tails)
        else:
            return self.score_func.gamma - batched_l2_dist(heads, tails)

    def head_neg_prepare(self, rel_id, num_chunks, head, tail, gpu_id, trace=False):
        return head, tail

    def tail_neg_prepare(self, rel_id, num_chunks, head, tail, gpu_id, trace=False):
        return head, tail
    '''
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.zeros_(self.proj_layer.bias)
    ...
    def forward_test_demo(self, pos_g, neg_g, id2entity_map, id2relation_map, wiki_name, logs, gpu_id=-1):
        """Do the forward and generate ranking results.
        ...
        print('neg_g.neg_head = {}'.format(neg_g.neg_head))
        # if neg_g.neg_head:
            # return
        '''
        print('pos_g.ndata[id] = {}'.format(pos_g.ndata['id'].size()))

        print('neg_g.ndata[id] = {}'.format(neg_g.ndata['id'].size()))
        # print(neg_g.ndata['id'])

        print('pos_scores = {}'.format(pos_scores.size()))
        '''
        # print(dir(pos_g))
        # print('pos_g.edges() = {}'.format(pos_g.edges()))
        # print('neg_g.edges() = {}'.format(neg_g.edges()))

        head_ids = [pos_g.ndata['id'][x].item() for x in pos_g.edges()[0].tolist()]
        tail_ids = [pos_g.ndata['id'][x].item() for x in pos_g.edges()[1].tolist()]
        rel_ids = pos_g.edata['id'].tolist()
        '''
        print('head_ids = {}'.format(head_ids))
        print('tail_ids = {}'.format(tail_ids))
        print('rel_ids = {}'.format(rel_ids))
        
        for h, r, t in zip(head_ids, rel_ids, tail_ids):
            print('{} {} {}'.format(id2entity_map[h], id2relation_map[r], id2entity_map[t]))
        '''
        # print(type(id2entity_map))
        # print(pos_g.ndata.keys())
        # print(pos_g.edata.keys())
        # print([id2entity_map[x.item()] for x in pos_g.ndata['id']])

        neg_scores = self.predict_neg_score(pos_g, neg_g, to_device=cuda,
                                            gpu_id=gpu_id, trace=False,
                                            neg_deg_sample=self.args.neg_deg_sample_eval)
        
        # print('neg_scores = {}'.format(neg_scores.size()))
        ...
        if neg_g.neg_head:
            if self.args.neg_deg_sample_eval:
                neg_node_ids = torch.cat((pos_g.all_edges(order='eid')[0], neg_g.ndata['id'][neg_g.head_nid]))
            else:
                neg_node_ids = neg_g.ndata['id'][neg_g.head_nid]
        else:
            if self.args.neg_deg_sample_eval:
                neg_node_ids = torch.cat((pos_g.all_edges(order='eid')[1], neg_g.ndata['id'][neg_g.tail_nid]))
            else:
                neg_node_ids = neg_g.ndata['id'][neg_g.tail_nid]

        print('neg_node_ids = {}'.format(neg_node_ids.size()))
        # print(neg_g.ndata['id'].size())
        # print(len([id2entity_map[neg_node_ids[i].item()] for i in range(neg_node_ids.size(0))]))
        print([id2entity_map[neg_node_ids[i].item()] for i in range(neg_node_ids.size(0))])
        
        higher_ranked_list = [id2entity_map[neg_node_ids[i].item()] for i in range(neg_node_ids.size(0)) if neg_scores[batch_idx, i] > pos_scores[batch_idx]]
        
        print('higher_ranked_list = {}'.format(higher_ranked_list))
        
        # print('len(higher_ranked_list) = {}'.format(len(higher_ranked_list)))
        
        tmp = [i for i in range(neg_node_ids.size(0)) if neg_scores[batch_idx, i] >= pos_scores[batch_idx]]
        # print(neg_scores[batch_idx, :])
        # print(pos_scores[batch_idx])
        tmp_scores = torch.tensor([neg_scores[batch_idx, i] for i in range(neg_node_ids.size(0)) if neg_scores[batch_idx, i] >= pos_scores[batch_idx]])

        if len(tmp_scores)>0:
            topk_idx = min(topk_idx, max(1, tmp_scores.size(0)))
            print('topk_idx = {}'.format(topk_idx))
            # print(tmp_scores)
            tmp2, top5_idx = torch.topk(tmp_scores, k=topk_idx)

            # print(tmp2)
            higher_ranked_list2 = [id2entity_map[neg_node_ids[tmp[x]].item()] for x in top5_idx]
            # print(type(higher_ranked_list2))
            # print(type(wiki_name))
            # print(higher_ranked_list2)
            if wiki_name:
                print('top {} higher ranked entities = {}'.format(topk_idx, [wiki_name[x] for x in higher_ranked_list2 if x in wiki_name]))
        ...
        for h, r, t, rank in zip(head_ids, rel_ids, tail_ids, rankings):
            print('{} {} {} {}'.format(id2entity_map[h], id2relation_map[r], id2entity_map[t], rank))
