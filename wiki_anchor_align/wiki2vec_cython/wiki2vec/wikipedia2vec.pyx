...
@cython.initializedcheck(False)
@cython.cdivision(True)
def train_page_custom(Paragraph paragraph, set entity_indices, int32_t window_orig, DumpDB dump_db, Dictionary dictionary, MentionDB mention_db, BaseTokenizer tokenizer, np.ndarray sample_ints):
    # print('flag 1')
    cdef bint matched
    cdef int32_t i, j, n, start, end, span_start, span_end, ent_start, ent_end, word, word2,\
        entity, text_len, token_len
    cdef int32_t [:] words, word_pos, sent_char_pos, sent_token_pos
    cdef char [:] link_flags
    cdef const int32_t [:] neighbors
    cdef unicode text
    ...
    for (entity, ent_start, ent_end) in target_links:
        ...
        # window = randint_c() % params.window + 1
        window = random.randint(0, sys.maxsize) % window_orig + 1
        ...
        for j in range(start, end):
            ...
            if sample_ints[word2] < random.random():
                continue
                
            word_entity_tuples.append((word2, entity))
            entity_word_tuples.append((entity, word2))

    return (word_entity_tuples, entity_word_tuples)
