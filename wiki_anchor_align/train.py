...
def main():
    ...
    for iter_id in range(args.start_epoch, args.n_iters):
        ...
        # create a copy of emb0 tensor
        sg_model_emb0_cp = deepcopy(sg_model.emb0_lookup.weight.data)

        # copy embeddings from transE to skip-gram copy
        sg_model_emb0_cp[common_entity_sg_idx] = model.entity_emb.emb[common_node_ids_tensor]

        # dump it to npy file
        sg_emb0_fname = os.path.join(args.save_path, args.dataset+'_'+model.model_name+'_emb0_cp_sg'+'.npy')
        np.save(sg_emb0_fname, sg_model_emb0_cp.cpu().numpy())

        sg_emb1_fname = os.path.join(args.save_path, args.dataset+'_'+model.model_name+'_emb1_sg'+'.npy')
        np.save(sg_emb1_fname, sg_model.emb1_lookup.weight.data.cpu().numpy())

        subprocess.run(['wikinew_word_ent', 'train-embedding', args.dump_db_file, args.dictionary_file, sg_emb0_fname, sg_emb1_fname, str(args.n_iters), str(iter_id), str(args.balance_param), os.path.join(args.save_path, 'emb_file'), '--pool-size', str(args.num_proc_train), '--dim-size', str(args.hidden_dim), '--mention-db', args.mention_db_file, '--link-graph', args.link_graph_file])
        print('Iteration {} of skip-gram model completed in {} sec.'.format(iter_id, time.time()-epoch_start_time))

        # copy skip-gram model weights from 'model_file' to PyTorch model
        emb_combined = joblib.load(os.path.join(args.save_path, 'emb_file'))
        sg_model_emb0_cp.copy_(torch.tensor(emb_combined['syn0']))
        sg_model.emb0_lookup.weight.data.copy_(torch.tensor(emb_combined['syn0']))
        sg_model.emb1_lookup.weight.data.copy_(torch.tensor(emb_combined['syn1']))
        ...
        model.entity_emb.emb[common_node_ids_tensor] = sg_model_emb0_cp[common_entity_sg_idx]
