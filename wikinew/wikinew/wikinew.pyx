...
cdef class Wikipedia2Vec:
    ...
    def train(self, dump_db, emb0_ckpt_file, emb1_ckpt_file, n_total_iters, iter_id, link_graph, mention_db, tokenizer, sentence_detector, pool_size,
        ...
        logger.info('Total number of word occurrences: %d', total_word_count * n_total_iters)
        ...
        total_page_count = dump_db.page_size() * n_total_iters
        ...
        self.syn0[:] = np.load(emb0_ckpt_file)
        self.syn1[:] = np.load(emb1_ckpt_file)
        ...
        with closing(Pool(pool_size, initializer=init_worker, initargs=init_args)) as pool:
            ...
            with tqdm(total=len(titles), mininterval=0.5, disable=not progressbar,
                      desc='Iteration %d/%d' % (iter_id + 1, n_total_iters)) as bar:
                for _ in pool.imap_unordered(train_page, args_generator(titles, iter_id),
        ...
        train_params = dict(
            ...
            version=pkg_resources.get_distribution('wikinew').version,
        )
