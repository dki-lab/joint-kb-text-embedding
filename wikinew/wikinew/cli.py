...
from .wikinew import Wikipedia2Vec
...
@click.argument('dump_db_file', type=click.Path(exists=True))
@click.argument('dictionary_file', type=click.Path(exists=True))
@click.argument('emb0_ckpt_file', type=click.Path())
@click.argument('emb1_ckpt_file', type=click.Path())
@click.argument('n_total_iters', type=click.INT)
@click.argument('iter_id', type=click.INT)
@click.argument('out_file', type=click.Path())
...
@train_embedding_options
@common_options
def train_embedding(dump_db_file, dictionary_file, emb0_ckpt_file, emb1_ckpt_file, n_total_iters, iter_id, link_graph, mention_db, tokenizer, sent_detect,
                    out_file, **kwargs):
    dump_db = DumpDB(dump_db_file)
    dictionary = Dictionary.load(dictionary_file)
    ...
    wiki2vec.train(dump_db, emb0_ckpt_file, emb1_ckpt_file, n_total_iters, iter_id, link_graph, mention_db, tokenizer, sent_detect, **kwargs)
