...
do
	PYTHONHASHSEED=1234 python utils/analogy/eval_analogy.py --sg-ckpt-file $SAVE_DIR/FB15k_TransE_l2_emb0_sg_ent.bin --analogy-file analogy_dataset_50rels/${rel}.txt --entity-counter-file $WIKIPEDIA_PROC_DATA/ent_counter_names.json --wiki-link-file ~/data/wikidata_dec_20_proc_json/wikipedia_links.json --entity-file $WIKIDATA_TRIPLES_DIR/entities.tsv --degree-sampling > analogy_dataset_50rels/results/${rel}.txt &
done
